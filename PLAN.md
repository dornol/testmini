...
- [ ] `oidc_provider` 테이블 정의 (`src/lib/server/db/schema.ts`)
  - `id`, `name` (표시명), `slug` (고유 식별자, URL-safe)
  - `provider_type`: OIDC, OAUTH2, SAML (향후)
...
- [ ] `oidc_account` 테이블 정의 (외부 IdP ↔ 내부 user 연동)
  - `id`, `user_id` (FK → user), `provider_id` (FK → oidc_provider)
  - `external_id` (IdP에서의 사용자 ID)
  - `email`, `name` (IdP에서 받은 정보)
  - `access_token`, `refresh_token` (암호화 저장, 선택)
  - `created_at`, `updated_at`
  - UNIQUE(`provider_id`, `external_id`)
- [ ] `pnpm db:push`로 반영

#### 11.2 OIDC Provider 관리 API

- [ ] `src/routes/api/admin/oidc-providers/+server.ts`
  - GET — Provider 목록 (ADMIN only)
  - POST — Provider 추가 (OIDC Discovery 자동 설정 지원)
- [ ] `src/routes/api/admin/oidc-providers/[providerId]/+server.ts`
  - GET — Provider 상세
  - PATCH — Provider 수정
  - DELETE — Provider 삭제 (연동된 계정이 있으면 비활성화만)
- [ ] OIDC Discovery 자동 검색 (`/.well-known/openid-configuration` 파싱)
- [ ] Client secret 암호화 유틸리티 (`src/lib/server/crypto.ts`)

#### 11.3 Custom OAuth Handler

- [ ] `src/routes/auth/oidc/[slug]/+server.ts` — Authorization 리다이렉트
  - DB에서 provider 설정 읽기
  - PKCE (code_verifier/code_challenge) 생성
  - state 파라미터에 CSRF 토큰 포함
  - IdP authorization endpoint로 리다이렉트
- [ ] `src/routes/auth/oidc/[slug]/callback/+server.ts` — Callback 처리
  - Authorization code → token exchange
  - ID token 검증 (issuer, audience, expiry, nonce)
  - Userinfo endpoint에서 프로필 조회
  - `oidc_account` 조회 → 기존 연동 있으면 로그인
  - 연동 없으면: `auto_register` 시 계정 생성, 아니면 에러
  - better-auth 세션 발급
- [ ] PKCE + state 검증으로 CSRF/코드 가로채기 방지

#### 11.4 Admin UI — OIDC Provider 관리

- [ ] `src/routes/admin/oidc-providers/+page.svelte` — Provider 목록
  - 테이블 (이름, 타입, 활성 상태, 연동 계정 수)
  - 활성/비활성 토글
- [ ] `src/routes/admin/oidc-providers/new/+page.svelte` — Provider 추가 폼
  - issuer URL 입력 → Discovery 자동 검색 버튼
  - 수동 설정 (authorization/token/userinfo URL)
  - Client ID/Secret 입력
  - 스코프, 자동 등록 여부, 표시 순서 설정
- [ ] `src/routes/admin/oidc-providers/[providerId]/+page.svelte` — Provider 수정
- [ ] 연결 테스트 버튼 (설정 검증)

#### 11.5 Login 페이지 통합

- [ ] 로그인 페이지에 활성화된 OIDC provider 버튼 동적 표시
  - DB에서 `enabled` provider 목록 조회 (`+page.server.ts`)
  - 각 provider별 "[아이콘] [이름]으로 로그인" 버튼
  - 이메일/비밀번호 로그인과 구분선 ("또는")
- [ ] 회원가입 페이지에도 동일하게 OIDC 버튼 표시

#### 11.6 계정 연동 관리 (사용자)

- [ ] 사용자 프로필/설정 페이지에서 연동된 IdP 목록 표시
- [ ] 추가 IdP 연동 / 연동 해제 기능
- [ ] 이메일 기반 자동 계정 매칭 (같은 이메일이면 기존 계정에 연동)
