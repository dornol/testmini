...
vi.mock('$lib/server/db/schema', () => ({
	testCaseGroup: { id: 'id', projectId: 'project_id', sortOrder: 'sort_order' }
}));
vi.mock('drizzle-orm', () => ({
...
describe('/api/projects/[projectId]/test-case-groups/reorder', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	describe('PUT', () => {
		...
		it('should return 400 for non-integer sortOrder', async () => {
			const event = createMockEvent({
				method: 'PUT',
				params: PARAMS,
				body: { groups: [{ id: 1, sortOrder: 1.5 }] },
				user: adminUser
			});
			const response = await PUT(event);
		...
		it('should return 400 when groups do not belong to project', async () => {
			// Return fewer groups than requested, simulating some not belonging to the project
			mockSelectResult(mockDb, [{ id: 1 }]);

			const event = createMockEvent({
				method: 'PUT',
				params: PARAMS,
				body: { groups: [{ id: 1, sortOrder: 0 }, { id: 999, sortOrder: 1 }] },
				user: adminUser
			});
			const response = await PUT(event);
			...
			expect(response.status).toBe(400);
			const data = await response.json();
			expect(data.error).toBe('Some groups do not belong to this project');
		});
	});
});
