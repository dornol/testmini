...
describe('/api/projects/[projectId]/test-runs/[runId]', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	describe('PATCH', () => {
		...
		it('should return 409 when run status is not CREATED', async () => {
		...
		it('should update run on success', async () => {
			mockDb.query.testRun.findFirst.mockResolvedValue(sampleTestRun);
			const updateChain = {
		...
		it('should update environment on success', async () => {
			mockDb.query.testRun.findFirst.mockResolvedValue(sampleTestRun);
			const updateChain = {
				set: vi.fn().mockReturnThis(),
				where: vi.fn().mockReturnThis(),
	...
	describe('DELETE', () => {
		...
		it('should delete run on success', async () => {
			mockDb.query.testRun.findFirst.mockResolvedValue(sampleTestRun);
			const deleteChain = {
				where: vi.fn().mockReturnThis(),
				then: (r: (v: unknown) => void) => Promise.resolve(undefined).then(r)
